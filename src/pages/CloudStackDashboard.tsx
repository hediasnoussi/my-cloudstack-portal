...
const CloudStackDashboard = ({ onLogout }: { onLogout: () => void }) => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Récupérer les vraies données CloudStack
      const cloudstackStats = await cloudstackService.getDashboardStats();
      
      console.log('✅ Données CloudStack récupérées:', cloudstackStats);
      
      setStats({
        domains: cloudstackStats.domains || 0,
        users: 0, // Pas d'utilisateurs dans les stats CloudStack
        accounts: cloudstackStats.accounts || 0,
        projects: cloudstackStats.projects || 0,
        instances: cloudstackStats.instances || 0,
        volumes: cloudstackStats.volumes || 0,
        networks: cloudstackStats.networks || 0,
        securityGroups: cloudstackStats.securityGroups || 0
      });
    } catch (error) {
      console.error('❌ Erreur lors du chargement des données CloudStack:', error);
