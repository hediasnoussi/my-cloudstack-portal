...
const Volumes = () => {
  ...
  const fetchVolumes = async () => {
    try {
      ...
      const transformedVolumes = cloudstackVolumes.map(volume => ({
        id: volume.id,
        name: volume.name || volume.displayname,
        size: volume.size || '0 GB',
        type: volume.type || 'data',
        zone: volume.zonename || 'N/A',
        state: volume.state || 'unknown',
        attached_to: volume.vmname || null,
        created_at: volume.created || new Date().toISOString()
      }));
      
      setVolumes(transformedVolumes);
      console.log('✅ Volumes transformés:', transformedVolumes);
    } catch (err) {
  ...
  const handleSubmit = async () => {
    try {
      if (dialogMode === 'create') {
        ...
        const zones = await cloudstackService.getZones();
        if (zones.length === 0) {
          throw new Error('Aucune zone disponible dans CloudStack');
        }

        // Préparer les données pour la création
        const volumeData = {
          name: formData.name,
          size: parseInt(formData.size) || 10, // Valeur par défaut si invalide
          zoneid: zones[0].id, // Utiliser la première zone disponible
          diskofferingid: null // Optionnel
        };

        console.log('🚀 Création du volume avec les données:', volumeData);

        // Créer le volume via l'API CloudStack
        const result = await cloudstackService.createVolume(volumeData);

        console.log('✅ Volume créé avec succès:', result);

        // Recharger la liste des volumes
        await fetchVolumes();

        setSnackbar({ open: true, message: t('volumes.volumeCreatedSuccess'), severity: 'success' });
      } else {
        // Pour l'instant, la mise à jour n'est pas implémentée dans CloudStack
        setSnackbar({ open: true, message: 'Mise à jour non supportée pour les volumes CloudStack', severity: 'warning' });
      }
      handleCloseDialog();
    } catch (err) {
      console.error('❌ Erreur lors de la création du volume:', err);
  ...
  const handleDelete = async (id) => {
    try {
      console.log('🗑️ Suppression du volume CloudStack:', id);
      
      // Supprimer le volume via l'API CloudStack
      await cloudstackService.deleteVolume(id);
      ...
      console.error('❌ Erreur lors de la suppression du volume:', err);
      setSnackbar({ open: true, message: `${t('volumes.errorDeletingVolume')}: ${err.message}`, severity: 'error' });
    }
  };

  const handleAttach = async (volumeId) => {
    try {
      console.log('🔗 Attachement du volume CloudStack:', volumeId);
      
      // Pour l'instant, on simule l'attachement car il faut choisir une VM
      // TODO: Ajouter un dialogue pour choisir la VM
      setSnackbar({ open: true, message: 'Fonctionnalité d\'attachement à implémenter', severity: 'info' });
      
      // Recharger la liste des volumes
      await fetchVolumes();
      
    } catch (err) {
      console.error('❌ Erreur lors de l\'attachement du volume:', err);
      setSnackbar({ open: true, message: `${t('volumes.errorAttachingVolume')}: ${err.message}`, severity: 'error' });
    }
  };

  const handleDetach = async (volumeId) => {
    try {
      console.log('🔓 Détachement du volume CloudStack:', volumeId);
      
      // Détacher le volume via l'API CloudStack
      await cloudstackService.detachVolume(volumeId);
      ...
      console.error('❌ Erreur lors du détachement du volume:', err);
