...
const Compute = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        console.log('🔄 Chargement des données Compute CloudStack...');
        
        // Charger les instances CloudStack
        ...
        // Transformer les données CloudStack
        const transformedInstances = cloudstackInstances.map(instance => ({
          id: instance.id,
          name: instance.name || instance.displayname,
          state: instance.state,
          internalName: instance.instancename,
          ipAddress: instance.nic?.[0]?.ipaddress || 'N/A',
          arch: instance.arch || 'x86_64',
          host: instance.hostname || 'N/A',
        ...
        console.error('❌ Erreur lors du chargement des données Compute:', err);
  ...
  return (
    <div className="w-full">
      ...
      <div className="border-black/12.5 dark:bg-slate-850 dark:shadow-dark-xl shadow-xl relative z-20 flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border mb-6">
        <div className="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid p-6 pt-4 pb-0">
          <div className="flex flex-wrap">
            {[
              { label: 'VPS', icon: 'fas fa-server', index: 0 },
        ...
        <div className="border-black/12.5 dark:bg-slate-850 dark:shadow-dark-xl shadow-xl relative z-20 flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border">
          <div className="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid p-6 pt-4 pb-0">
            <div className="flex justify-between items-center">
              <h6 className="capitalize dark:text-white">VPS ({instances.length})</h6>
      ...
      <TabPanel value={tabValue} index={1}>
        <div className="border-black/12.5 dark:bg-slate-850 dark:shadow-dark-xl shadow-xl relative z-20 flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border">
          <div className="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid p-6 pt-4 pb-0">
                          <h6 className="capitalize dark:text-white">VPS Snapshots ({instanceSnapshots.length})</h6>
          </div>
          <div className="flex-auto p-4">
            <div className="overflow-x-auto">
              <table className="w-full">
